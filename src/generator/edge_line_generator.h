...
private:
    ...
    // -----------------------------------------------
    void bezierAlignEnds(std::vector<Vec2d>& pts,
        const Vec2d& startPt, const Vec2d& startTang,
        const Vec2d& endPt,   const Vec2d& endTang,
        int K) const
    {
        int n = (int)pts.size();
        if(n < 6) return;
        ...
        // Head refit [0..K]
        {
            ...
            if(d > EPS){
                ...
                BezierSegment cb; cb.ctrl = {p0, p1, p2, p3};
                for(int i=1; i<K; ++i){
                    double t = (double)i / K;
                    pts[i] = cb.evaluate(t);
                }
    ...
    // -----------------------------------------------
    Vec2d findEdgePtInGroup(const std::string& lineId, bool isLeft,
