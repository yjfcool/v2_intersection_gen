...
private:
    ...
    std::vector<SiblingCurve> buildSiblings(
        ...
        const ClusterOrderSolver& cs) const;

    std::vector<SiblingCurve> buildSiblingsOptimized(
        const ConnId& id, const std::unordered_map<ConnId, BezierCurve>& done,
    ...
    BoundingBox2d getCurveBoundsForQuery(const Connectivity& conn, const IntersectionInput& input) const;

    // Helper methods for persistent spatial index
    void initializePersistentQuadTree(const IntersectionInput& input);
    void addCurveToPersistentQuadTree(const ConnId& id, const BezierCurve& curve);
    void clearPersistentSpatialIndex();
};
