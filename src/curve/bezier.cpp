...
std::pair<BezierSegment,BezierSegment> BezierSegment::splitAt(double t)const{
    ...
    BezierSegment L,R;
    L.ctrl={ctrl[0],p01,p012,p};R.ctrl={p,p123,p23,ctrl[3]};
    return{L,R};
}
BoundingBox2d BezierSegment::bbox()const{
    BoundingBox2d b;for(int i=0;i<=20;++i)b.expand(evaluate(i/20.0));return b;
}
double BezierSegment::arcLength(int s)const{
    double len=0;Vec2d prev=ctrl[0];
    for(int i=1;i<=s;++i){Vec2d c=evaluate((double)i/s);len+=(c-prev).norm();prev=c;}
    return len;
}
double BezierSegment::arcLengthToParam(double s,int samp)const{
    double tot=arcLength(samp),tgt=s*tot,acc=0;
    Vec2d prev=ctrl[0];double dt=1.0/samp;
    for(int i=1;i<=samp;++i){
        double t=i*dt;Vec2d cur=evaluate(t);double sl=(cur-prev).norm();
        if(acc+sl>=tgt){double f=(tgt-acc)/(sl+1e-12);return(i-1)*dt+f*dt;}
        acc+=sl;prev=cur;}
    return 1.0;
}
...
Vec2d BezierCurve::evaluate(double u)const{
    assert(!segs.empty());
    int n=(int)segs.size();double sc=u*n;int idx=std::min((int)sc,n-1);
    return segs[idx].evaluate(sc-idx);}
Vec2d BezierCurve::tangent(double u)const{
    assert(!segs.empty());
    int n=(int)segs.size();double sc=u*n;int idx=std::min((int)sc,n-1);
    Vec2d t=segs[idx].tangent(sc-idx);return t.norm()>1e-10?t.normalized():Vec2d(1,0);}
std::vector<Vec2d> BezierCurve::sample(int n)const{
    std::vector<Vec2d>out(n);
    for(int i=0;i<n;++i)out[i]=evaluate((double)i/(n-1));return out;}
double BezierCurve::arcLength()const{
    double t=0;for(auto&s:segs)t+=s.arcLength();return t;}
std::vector<Vec2d> BezierCurve::sampleByArcLength(int n)const{
    ...
    std::vector<Vec2d>out;out.push_back(startPt());
    double acc=0,tgt=step;
    for(auto&seg:segs){
        int sub=50;Vec2d prev=seg.ctrl[0];double dt=1.0/sub;
        for(int i=1;i<=sub;++i){
            Vec2d cur=seg.evaluate(i*dt);double d=(cur-prev).norm();
            while(acc+d>=tgt&&(int)out.size()<n-1){  // stop at n-1, reserve last slot
                double f=(tgt-acc)/(d+1e-12);out.push_back(prev+f*(cur-prev));tgt+=step;}
            acc+=d;prev=cur;}}
...
BoundingBox2d BezierCurve::bbox()const{
    BoundingBox2d b;
    for(auto&s:segs){auto sb=s.bbox();b.expand(sb.min_pt);b.expand(sb.max_pt);}
    return b;}
...
BezierSegment makeCubicG1(const Vec2d&p0,const Vec2d&t0,const Vec2d&p1,const Vec2d&t1,double alpha){
    double d=(p1-p0).norm();BezierSegment s;
    s.ctrl[0]=p0;s.ctrl[1]=p0+alpha*d*t0.normalized();
    s.ctrl[2]=p1-alpha*d*t1.normalized();s.ctrl[3]=p1;return s;}
...
static Vec2d catmullTan(const std::vector<Vec2d>&pts,int i){
    int n=(int)pts.size();if(n<2)return Vec2d(1,0);
...
BezierCurve fitBezierWithEndTangents(const std::vector<Vec2d>&pts,const Vec2d&st,const Vec2d&et){
    ...
    tans.front()=st.normalized();tans.back()=et.normalized();
...
AdaptiveRefineResult adaptiveRefine(const BezierCurve&c,const SDFField&sdf,double kmax,double ptol){
    AdaptiveRefineResult res;res.curve.segs=c.segs;
    for(int iter=0;iter<4;++iter){
        std::vector<BezierSegment>next;bool split=false;
...
BezierCurve curveFromParamsFull(const VecXd&params,const BezierCurve&proto){
    ...
    BezierCurve c;c.segs=proto.segs;
    if(n==0)return c;
