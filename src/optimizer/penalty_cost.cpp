...
double PenaltyCost::evalFence(const BezierCurve& c) const {
    if (cache_.fence_empty) return 0;
    double cost = 0;
    constexpr int S = 20;
    for (auto& seg : c.segs)
        for (int i = 0; i <= S; ++i) {
    ...
    for (auto& cw : crosswalks) {
        for (auto& seg : c.segs)
            for (int i = 1; i < 20; ++i) {
                ...
                if (!polygonContains(cw.geometry, pt)) continue;
...
// ─────────────────────────────────────────────────────────────────────────────
static void enforceEndpointG1(BezierCurve& c,
...
// ─────────────────────────────────────────────────────────────────────────────
void PenaltyCost::addObstacleGrad(const BezierCurve& c, const VecXd& params,
                                    double w, VecXd& grad) const
{
    if (!sdf || w < 1e-15) return;
    constexpr int S = 20;
    ...
    for (int k = 0; k < n_segs; ++k) {
        ...
        for (int i = 0; i <= S; ++i) {
            ...
            if (slack >= 0) continue;           // no violation → zero gradient
...
// ─────────────────────────────────────────────────────────────────────────────
void PenaltyCost::addSmoothGrad(const BezierCurve& c, const VecXd& params,
                                  double w, VecXd& grad) const
{
    if (w < 1e-15) return;
    constexpr int S = 12;
    ...
    for (int k = 0; k < n_segs; ++k) {
        ...
        for (int j = 0; j < 4; ++j) {   // 4 params per segment
            // Perturb only the j-th param of segment k (sparse structure)
            VecXd pp = params, pm = params;
            pp[base+j] += h; pm[base+j] -= h;

            auto cp = full_param_mode ? curveFromParamsFull(pp,proto)
                                      : curveFromParams(pp,proto);
            auto cm = full_param_mode ? curveFromParamsFull(pm,proto)
                                      : curveFromParams(pm,proto);
...
// ─────────────────────────────────────────────────────────────────────────────
void PenaltyCost::addNumericGrad(const VecXd& params,
                                   double w_bnd, double w_fence,
                                   double w_cluster, double w_xwalk,
                                   VecXd& grad)
{
    ...
    if (!need_bnd && !need_fence && !need_cluster && !need_xwalk) return;
    ...
    for (int i = 0; i < n; ++i) {
        double orig = p[i];
        p[i] = orig + h;
        auto cp = full_param_mode ? curveFromParamsFull(p,proto)
                                  : curveFromParams(p,proto);
        enforceEndpointG1(cp, start_tan_dir, end_tan_dir);
        double fp = 0;
        ...
        p[i] = orig - h;
        auto cm = full_param_mode ? curveFromParamsFull(p,proto)
                                  : curveFromParams(p,proto);
        enforceEndpointG1(cm, start_tan_dir, end_tan_dir);
        double fm = 0;
...
double PenaltyCost::operator()(const VecXd& params, VecXd& grad)
{
    ...
    BezierCurve c = full_param_mode
                  ? curveFromParamsFull(params, proto)
                  : curveFromParams    (params, proto);
    ...
    addNumericGrad(params,
                   weights.boundary, weights.fence,
                   weights.cluster,  weights.crosswalk,
                   grad);
                           ...
                           const BezierCurve& initial, int outer_iters)
{
    ...
    VecXd params = cost.full_param_mode
                 ? curveToParamsFull(initial)
                 : curveToParams(initial);

    for (int outer = 0; outer < outer_iters; ++outer) {
        auto res = solver.solve(
            [&](const VecXd& p, VecXd& g) { return cost(p, g); },
            params);
        params = res.x;
        ...
        if (op + bp + fp + cp < 1e-6) break;   // all constraints satisfied
    }
    return cost.full_param_mode
         ? curveFromParamsFull(params, cost.proto)
         : curveFromParams(params, cost.proto);
}
