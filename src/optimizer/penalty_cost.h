...
// ─────────────────────────────────────────────────────────────────────────────
struct BoundarySegment { Vec2d a, b; };
...
struct PenaltyCostCache {
    ...
    struct SiblingPoly { std::vector<Vec2d> pts; bool exempt_a1; double a2_radius; };
    std::vector<SiblingPoly> sib_polys;

    // Fence polygon winding-number lookup cached at construction
    bool fence_empty = true;

    void rebuild(const std::vector<Boundary>&    boundaries,
...
private:
    ...
    void addObstacleGrad(const BezierCurve& c, const VecXd& params,
    ...
    void addNumericGrad(const VecXd& params, double w_bnd, double w_fence,
                         double w_cluster, double w_xwalk, VecXd& grad);

    // Smooth term analytic gradient via curvature derivative
    void addSmoothGrad(const BezierCurve& c, const VecXd& params,
                        double w, VecXd& grad) const;
};

BezierCurve optimiseCurve(PenaltyCost& cost, LBFGSSolver& solver,
                           const BezierCurve& initial, int outer_iters = 4);
