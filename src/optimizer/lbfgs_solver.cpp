...
static double cubicMinimiser(double a, double fa, double ga,
...
double LBFGSSolver::lineSearch(CostFn& fn,
                                const VecXd& x, const VecXd& dir,
                                double f0, const VecXd& g0,
                                VecXd& xn, double& fn_val, VecXd& gn,
                                int& evals)
{
        ...
        if (armijo_fail || non_descent) {
            ...
            for (int z = 0; z < 10; ++z) {
                ...
                if (f_j > f0 + c1*a_j*derphi0 || f_j >= f_lo) {
                    a_hi = a_j; g_hi = g_cur; fn_val = f_j;
                } else {
                    double dp = g_cur.dot(dir);
                    if (std::abs(dp) <= -c2 * derphi0) {
                        gn = g_cur; fn_val = f_j; return a_j;
                    }
                    f_lo = f_j; g_lo = g_cur;
                    if (dp * (a_hi - a_lo) >= 0) a_hi = a_lo;
                    a_lo = a_j;
                }
            }
            // Return best found so far
            gn = g_cur; fn_val = f_lo; return a_lo;
        }

        double dp = g_cur.dot(dir);
        if (std::abs(dp) <= -c2 * derphi0) {
            // Strong Wolfe satisfied
            gn = g_cur; fn_val = f_cur; return alpha;
        }
        if (dp >= 0) {
            ...
            for (int z = 0; z < 8; ++z) {
                ...
                if (f_j > f0 + c1*a_j*derphi0 || f_j >= f_lo) {
                    ...
                    f_lo = f_j; g_lo = g_cur; a_lo = a_j;
                }
            }
            gn = g_lo; fn_val = f_lo; return a_lo;
        }

        alpha_prev = alpha; f_prev = f_cur;
...
SolveResult LBFGSSolver::solve(CostFn fn, const VecXd& x0)
{
    ...
    for (int iter = 0; iter < cfg_.max_iter; ++iter) {
        ...
        if (g.norm() < cfg_.grad_tol) { res.converged = true; break; }
        ...
        if (sy > 1e-20) {   // skip update if curvature condition not met
            if ((int)sv.size() == cfg_.history_size) {
                sv.pop_front(); yv.pop_front(); rho.pop_front();
            }
            sv.push_back(s); yv.push_back(y); rho.push_back(1.0 / sy);
        }

        double f_diff = std::abs(fn_val - f);
        x = xn; g = gn; f = fn_val;

        if (f_diff < cfg_.func_tol) { res.converged = true; break; }
    }
