...
public:
    void build(const std::vector<Connectivity>&,const std::vector<Lane>&,
               const std::vector<LaneGroup>&);
    ...
    void checkAndMarkA2(const std::unordered_map<ConnId,BezierCurve>&,
                        const SDFField&,double r=1.5);
private:
