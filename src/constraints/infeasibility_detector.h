...
struct FenceRelaxResult{bool success=false;Polygon2d relaxed_fence;double max_expansion=0;};
FenceRelaxResult tryRelaxFence(const Polygon2d&,const std::vector<Boundary>&,
                                const Vec2d&,double mx=1.5);
ConnectivityCurve makeFallbackCurve(const PreCheckResult&,const Connectivity&,
                                     const Vec2d&,const Vec2d&);
...
inline PreCheckResult preCheck(const SDFField&sdf,const Polygon2d&fence,
                                const Vec2d&ep,const Vec2d&xp,double rw){
