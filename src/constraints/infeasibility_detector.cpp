...
bool segmentsIntersect2(const Vec2d&a,const Vec2d&b,const Vec2d&c,const Vec2d&d){
    Vec2d r=b-a,s=d-c;double den=cross2d(r,s);
    if(std::abs(den)<1e-12)return false;
    Vec2d ac=c-a;double t=cross2d(ac,s)/den,u=cross2d(ac,r)/den;
...
bool isSimplePolygon(const Polygon2d&poly){
    ...
    for(int i=0;i<n;++i)for(int j=i+2;j<n;++j){
        ...
        if(segmentsIntersect2(ring[i],ring[(i+1)%n],ring[j],ring[(j+1)%n]))return false;}
    return true;}
    ...
    const Vec2d&start,const Vec2d&goal,double mc)
{
    ...
    auto[sr,sc_]=sdf.worldToCell(start);auto[gr,gc_]=sdf.worldToCell(goal);
    int rows=sdf.rows(),cols=sdf.cols();
    auto key=[&](int r,int c){return r*cols+c;};
    std::unordered_set<int>vis;std::queue<std::pair<int,int>>q;
    auto push=[&](int r,int c){
        if(r<0||r>=rows||c<0||c>=cols)return;
        if(vis.count(key(r,c)))return;
        Vec2d w=sdf.cellToWorld(r,c);
        auto[d,dummy]=sdf.queryWithGrad(w);if(d<mc)return;
        if(!fence.outer.empty()&&!polygonContains(fence,w))return;
        vis.insert(key(r,c));q.push({r,c});};
    push(sr,sc_);
    int dx[]={0,0,1,-1,1,1,-1,-1},dy[]={1,-1,0,0,1,-1,1,-1};
    while(!q.empty()){auto[r,c]=q.front();q.pop();
        if(r==gr&&c==gc_)return true;
        for(int d=0;d<8;++d)push(r+dx[d],c+dy[d]);}
    return false;}
bool detectSandwich(const SDFField&sdf,const Polygon2d&fence,double cl){
    if(!sdf.valid()||fence.outer.empty())return false;
    ...
    const Vec2d&,double mx)
{
    ...
    Polygon2d relaxed;for(auto&pp:inf[0])relaxed.outer.emplace_back(pp.x,pp.y);
    for(auto&bnd:edges){if(bnd.type!=BoundaryType::RoadEdge)continue;
        auto&pts=bnd.geometry.points;auto&ring=relaxed.outer;
        int nr=(int)ring.size(),nb=(int)pts.size();
        for(int i=0;i<nr;++i)for(int j=0;j+1<nb;++j)
    ...
    const Vec2d&ep,const Vec2d&xp,double rw,const std::vector<Boundary>&)
{
    ...
    double minW=1e18;
    for(int i=1;i<20;++i){double t=(double)i/20;Vec2d pt=(1-t)*ep+t*xp;
        if(!fence.outer.empty()&&!polygonContains(fence,pt))continue;
        auto[d,dummy]=sdf.queryWithGrad(pt);minW=std::min(minW,d*2.0);}
    r.min_corridor_width=minW;r.narrow_passage=(minW<rw*1.2);
    r.fence_sandwich=detectSandwich(sdf,fence,0.5);
    if(r.topological_block)r.type=ViolationInfo::InfeasibilityType::TopologicalBlock;
    else if(r.fence_sandwich)r.type=ViolationInfo::InfeasibilityType::Sandwich;
    else if(r.narrow_passage)r.type=ViolationInfo::InfeasibilityType::NarrowPassage;
    return r;}
    ...
    const Vec2d&p0,const Vec2d&p1)
{
    ...
    out.id=conn.id;out.entry_lane_id=conn.entry_lane_id;
    out.exit_lane_id=conn.exit_lane_id;out.turn_type=conn.turn_type;
    out.violation.type=pre.type;
    if(pre.type==ViolationInfo::InfeasibilityType::TopologicalBlock){
        out.curve=std::nullopt;out.status=CurveStatus::Infeasible;
        out.violation.reason="No obstacle-free path (BFS unreachable)";}
    else{
        Vec2d dir=(p1-p0);double d=dir.norm();
        if(d>1e-6)dir.normalize();else dir=Vec2d(1,0);
        BezierSegment seg;seg.ctrl[0]=p0;seg.ctrl[1]=p0+dir*(d/3);
        seg.ctrl[2]=p1-dir*(d/3);seg.ctrl[3]=p1;
        BezierCurve c;c.segs.push_back(seg);
        out.curve=c;out.status=CurveStatus::Degraded;
